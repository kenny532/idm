...
修环境检测全绿后闪退只显示「拒绝访问。」的问题（[#24](https://github.com/tytsxai/IDM-Activation-Script-Chinese/issues/24)）。**激活 / 冻结 / 重置 / 更新开关逻辑未改。**

### 修复
- **`IAS.cmd` 的 conhost 重入失败时回退到当前窗口继续**。交互模式下脚本会 `start conhost.exe` 再启一次自己以关闭 QuickEdit；在 Win11 25H2 或被安全软件/策略拦截时，`start` 失败只会打印系统文案「拒绝访问。」然后退出，用户来不及看菜单。现在：`start` 成功仍走原重入路径；失败则提示中文警告，并在当前窗口继续进菜单，不再裸闪退。
- **Null 服务探测不再往屏幕打 `STATE : 4 RUNNING`**。原先 `sc query Null | find` 未重定向，成功时会漏一行服务状态，容易和后续报错叠在一起误导排查。

### 文档
- README 常见问题新增 **Q17**：环境检测全绿后清屏出现「拒绝访问。」怎么办。

### 兼容性
- `IAS.cmd` 版本号 `1.4.3` → `1.4.4`。
- 菜单编号、参数、退出码语义不变；仅入口稳健性增强。

---

## v1.4.3 — 2026-08-01

围绕两个用户反馈做的小版本：主菜单不再说"支持最新版 IDM"，改成明确标出版本号；文档补上浏览器扩展报错的排查路径。**激活 / 冻结 / 重置 / 更新开关的逻辑一行未动。**

### 改动
- **主菜单显示明确的版本号，不再说"支持最新版"**（[#22](https://github.com/tytsxai/IDM-Activation-Script-Chinese/issues/22)）。原来的第一行是"此脚本可支持最新版本的 IDM"，而 IDM 更新很频繁、脚本未必同步，用户容易误以为当天发布的新版也已适配，激活失败后才发现。现在第一行改为 `脚本 <iasver> | 已适配 IDM <idmsupport> 及之前的 6.x 版本`，下面一行显示从 `IDMan.exe` 文件版本信息读出的**本机 IDM 版本**；两者主次版本不一致时多打一行提示，指向 README 常见问题 Q10。已适配版本由脚本头部新增的 `idmsupport` 变量单点维护，发版时与 `iasver` 一起更新。初值取 `6.43`：沿用 README 常见问题 Q10 一直以来的兼容性表述和 6.42 / 6.43 用户的实际反馈，**不是**本版新做的实机回归；今后要上调这个值，请先在对应版本的 IDM 上实测（见维护清单）。
- **`开始激活.cmd` 的环境自检打印本机 IDM 版本号**。检测到安装路径后多读一次文件版本，用户自查和提 Issue 时都能直接看到版本，不用再去 IDM 的"关于"里翻。读取失败只提示一行，不计入问题数、不阻断激活。

### 文档
- README 常见问题新增 **Q16：Edge / Chrome 里 IDM 扩展图标变灰带红叉**（[#23](https://github.com/tytsxai/IDM-Activation-Script-Chinese/issues/23)）。这类现象来自 IDM 自身的浏览器集成（扩展 ↔ 本机 IDM），脚本只写 `AdvIntDriverEnabled2=1`，不安装/不禁用任何扩展。按"IDM 是否在运行 → 扩展与 IDM 版本是否匹配 → IDM 选项里的集成开关 → IDM 是否被判假序列号 → 是否同一个 Windows 账户"排查，并说明反馈时需要附哪些信息。
- README 的菜单示意图与 Q10 同步更新为新的版本提示文案。

### 新增（维护侧）
- CI 增加**主菜单渲染冒烟**：用管道喂入 `0` 跑一次交互菜单，再按 GBK 解码断言版本提示行确实打印、且没有出现批处理语法错误。菜单是最常用的路径，此前完全没有被冒烟覆盖——这次的改动正好落在菜单里。

### 兼容性
- `IAS.cmd` 版本号 `1.4.2` → `1.4.3`。
- **菜单编号、参数、退出码语义全部不变**，只是菜单顶部多了 1～2 行版本信息。

---

## v1.4.2 — 2026-07-28

上线前的稳定性与可维护性补强。修掉三个「文档写了但实现对不上」的缺口，并给发布流程加上自动守卫。**激活 / 冻结 / 重置 / 更新开关的逻辑一行未动。**

### 修复
- **日志文件名不再退化成字面量**。`IAS.cmd` 里生成日志文件名的那几行原本写在 `if (...)` 代码块内，而 CMD 会在整块解析时一次性展开 `%_logstamp%`，取不到上一行刚写入的值——结果所有静默运行的日志都叫 `IAS-%_logstamp%.log`、追加进同一个文件，时间戳完全失效。现在改由 `:init_log` 子程序逐行执行，CI 里加了对应的回归断言。
- **`/log=路径` 真正可用**。README、`使用说明.txt`、`ARCHITECTURE.md`、维护清单里都写着 `/log=路径`，但参数解析只认不带路径的 `/log`，用户按文档传路径时日志会被静默写到 `%SystemRoot%\Temp` 而不是指定位置。现在新增 `:extract_logpath` 支持该写法：目录不存在会自动创建，写不进去会提示并回退默认位置（路径仍不能含空格，参数按空格切分）。这里不能靠原有的 `for %%A in (%_args%)` 循环取值——`for` 的集合解析把 `=` 也当分隔符，`/log=C:\x.log` 在那里已经被拆成 `/log` 和 `C:\x.log` 两个 token，所以改成直接从完整参数串里提取。
- **`开始激活.cmd` 提权时不再丢参数**。以非管理员身份带参数运行（如 `开始激活.cmd /frz /silent`）时，`Start-Process -Verb RunAs` 没有传 `-ArgumentList`，参数被静默丢弃，提权后的窗口弹出的是交互菜单。同时把路径和参数里的单引号按 PowerShell 规则转义，含 `'` 的目录（如 `D:\Tom's Tools\`）不再让提权语法出错。

### 改动
- **收尾时打印备份与日志位置**。激活/冻结/重置流程结束时（包括失败退出）会显示本次注册表备份的文件名模式和日志路径；未开日志且退出码非 0 时，提示可加 `/log` 重跑取证。以前失败了只能去翻文档猜路径。

### 新增（维护侧，不影响用户功能）
- `tools/verify-release.ps1`：CI 里守住发布包与仓库一致——`.sha256` 与实际哈希一致、包内运行时文件（`IAS.cmd` / `开始激活.cmd` / `使用说明.txt`）与仓库逐字节一致（不一致直接失败）、文档类文件不一致只告警、`iasver` 与 CHANGELOG 顶部版本号一致。发布包用的是固定文件名，「只改仓库、忘了重新打包」以前没有任何征兆。
- `tools/pack-release.ps1`：把发布打包变成一条可重复的命令（固定清单、GBK 条目名、自动重算 sha256）。CI 每次都会跑一遍并把结果作为 `release-bundle-rebuilt` artifact 上传，没有 Windows 的维护者可以直接下载使用。
- CI 冒烟扩到 `/act`、`/frz`（未装 IDM 时应返回 `1`）和日志路径两条；`.gitattributes` 补 `*.zip binary`、`*.sha256`、`*.ps1` 规则，避免任何换行/文本启发式动到发布包字节。

### 文档
- README / `README.en.md` / `使用说明.txt` / `llms.txt` / `ARCHITECTURE.md` / `docs/` 统一日志说明（默认落点、`/log=路径`、路径不能含空格），并说明备份和日志不会自动清理、可自行删除。
- 修正 `开始激活.cmd` 的退出码描述：只有「已经是管理员」时才透传 `IAS.cmd` 的退出码；需要提权时激活流程在新进程里跑，原窗口拿不到退出码。
- `CONTRIBUTING.md` 修正 `.txt` 的编码约束（`使用说明.txt` 是 UTF-8 + BOM，不是 GBK），补充 `.ps1` 约定与发布包相关的本地自检命令。

### 兼容性
- `IAS.cmd` 版本号 `1.4.1` → `1.4.2`，保持脚本自报版本与 tag、发布包一致。
- **菜单编号、参数、退出码语义全部不变**，激活/冻结/重置/更新开关的行为与 v1.4.1 完全一致。`/log=路径` 从「文档写了但不生效」变成真的生效，属于修复而非行为变更。
- 发布包已重打，包内 `IAS.cmd` / `开始激活.cmd` / `使用说明.txt` 与仓库逐字节一致（此前包里这三个文件还停留在改动前的版本，README 落后四次文档提交——正是本版新加的守卫抓出来的）。
