...
- **v1.4.3 起主菜单会直接标出"已适配 IDM `<版本>`"和"本机检测到的 IDM `<版本>`"**（`开始激活.cmd` 的环境自检里也会打印本机 IDM 版本），不再是模糊的"支持最新版"；两者不一致时菜单会给出提示，但不代表一定不能用——IDM 6.x 之间差异通常很小，可以照常尝试
...
**原因（v1.4.4 已修）：** 环境自检通过后会进入 `IAS.cmd`。交互模式为关闭 QuickEdit，会执行 `start conhost.exe` 再拉起一次脚本。Win11 25H2 或安全软件/策略拦截时，`start` 失败，cmd 只打印系统文案「拒绝访问。」（脚本本身没有这句），窗口很快关掉。

**你现在可以这样处理：**
1. **升级到 v1.4.4+**：重入失败会自动在当前窗口继续，并给出中文警告，不应再裸闪退。
...
- **修复环境检测全绿后「拒绝访问。」闪退**（[#24](https://github.com/tytsxai/IDM-Activation-Script-Chinese/issues/24)）：`IAS.cmd` 的 conhost 重入失败时回退到当前窗口继续；Null 服务探测不再泄漏 `STATE` 行。

### v1.4.3 - 2026-08-01

围绕两个用户反馈的小版本，**激活 / 冻结 / 重置 / 更新开关的逻辑一行未动**。

- **主菜单显示明确版本号**（issue #22）：第一行由"此脚本可支持最新版本的 IDM"改为 `脚本 <版本> | 已适配 IDM <版本> 及之前的 6.x 版本`，并显示从 `IDMan.exe` 读到的本机 IDM 版本，两者不一致时给出提示，避免"以为最新版都支持"。
- **`开始激活.cmd` 环境自检打印本机 IDM 版本号**：自查和提 Issue 都能直接看到版本。
- **常见问题新增 Q16**（issue #23）：Edge / Chrome 里 IDM 扩展图标变灰带红叉的排查路径。
- **CI 新增主菜单渲染冒烟**：交互菜单此前完全没有被覆盖，现在会断言版本提示行确实打印且无批处理语法错误。
...
- 当前版本 **v1.4.4**（2026-08-04），文档与运行时脚本包同步。核心功能：`[1]` 冻结、`[2]` 激活、`[3]` 重置、`[4]`/`[5]` 禁用/恢复 IDM 更新提示。
